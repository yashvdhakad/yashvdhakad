...
const Hero = () => {
  ...
  return (
    <section className="w-full mt-20 mb-60 flex flex-col lg:flex-nowrap flex-wrap justify-between items-center gap-6 select-none">
      <div className="p-6 rounded drop-shadow shadow bg-clip-text bg-1 bg-cover text-center flex flex-col gap-6 items-center">
        <h1 className="text-9xl font-bold tracking-tighter text-transparent">
          {heroData.h1}
        </h1>
        <h3 className="w-1/2 text-xl text-transparent">
          {heroData.h3}
        ...
        <button className="py-3 px-6 bg-slate-200 text-slate-950  font-semibold rounded transition-all hover:scale-95 focus:bg-slate-300 shadow-md hover:shadow-sm">
          Book a quick call ›
        </button>
      </div>
      <div className="w-1/2">
        <LogoMarquee speed={40} size={40} />
      </div>
    </section>
  );
};
